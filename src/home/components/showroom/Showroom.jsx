...
function Showroom(props) {
  ...
  const getCards = (carInfo) => {
    const cards = [];
    let carInfoToDisplay =
      carInfo.length >= 10 ? carInfo.slice(0, 10) : carInfo;
    carInfoToDisplay.forEach((info) => {
      cards.push(
        <div className="showroom__card-container">
          {info.isBestSeller && (
            <div className="item-bestseller">
              <i class="fa-solid fa-star"></i> Best Seller
            </div>
          )}
          <div
            className="showroom__card"
            onClick={() => dispatch(showPopup(info.id))}
          >
            <div className="showroom__card-icon-container">
              <ItemImage
                img={info.img}
                class="showroom__card-icon"
                type={info.type}
              />
            </div>
            ...
            {info.discount && (
              <div className="showroom__card-new-price">
                {info.discountedPrice} CAD
              </div>
            )}
            <div
              className={
                "showroom__card-price " + (info.discount ? "sale" : "")
              }
            >
              {info.price} CAD
            </div>
  ...
  return (
    props.data.length > 0 && (
      <section className="showroom" id={props.id}>
        ...
        <div className="showroom__card-list">{getCards(props.data)}</div>
      </section>
    )
  );
}
