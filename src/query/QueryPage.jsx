  ...
  const getDisplayData = () => {
    ...
    if (query.partDetailFR) {
      filteredData = filteredData.filter((d) =>
        d.type.includes(query.partDetailFR)
      );
    }

    if (query.partDetailLR) {
      filteredData = filteredData.filter((d) =>
        d.type.includes(query.partDetailLR)
      );
    }
  ...
  const getItems = () => {
    ...
    getDisplayData().forEach((d) => {
      ...
      items.push(
        <div
          className={
            "query__results-item" + (isInBasket ? " added-to-basket" : "")
          }
          onClick={() => dispatch(showPopup(d.id))}
        >
          <div className="query__results-item-img">
            <ItemImage
              img={d.img}
              class="query__results-item-img-element"
              type={d.type}
            />
          </div>
          <div className="query__results-item-label">
            ...
            {d.isBestSeller && (
              <div className="query__results-item-bestseller">
          ...
          <div className="query__results-item-price">
            {d.discount && (
              <div className="query__results-item-price-discount">
            ...
            <div
              className={
                "query__results-item-price-real" + (d.discount ? " sale" : "")
              }
            >
              {d.price} CAD
            </div>
